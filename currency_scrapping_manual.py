...
def scrape_website(bank_abbv_name, url, selected_cur_list = None):
    ...
    if bank_abbv_name == 'KBANK JOURNEY':
        ...
        exc_val_df["selling_rate"] = exc_val_df["selling_rate"].apply(lambda x:"{:.4f}".format(float(x)))
        ...
        exc_val_list = []
        for row in tbody:
            exc_val_list.append([bank_abbv_name
                                ,row.find_all("td")[0].text
                                ,"{:.4f}".format(float(row.find_all("td")[2].text))
                                ,"{:.4f}".format(float(row.find_all("td")[1].text))])
        ...
        for row in tbody:
            try:
                if row.find("h6").text == 'USD':
                    exc_val_dict[row.find("small").text] = ["{:.4f}".format(float(row.find_all("td")[3].text))
                                                            ,"{:.4f}".format(float(row.find_all("td")[5].text))]
                else:
                    exc_val_dict[row.find("h6").text] = ["{:.4f}".format(float(row.find_all("td")[3].text))
                                                        ,"{:.4f}".format(float(row.find_all("td")[5].text))]
            except:
        ...
        exc_val_df = pd.DataFrame(exc_val_list,columns = header)
        exc_val_df = selected_currency(exc_val_df,selected_cur_list)
        exc_val_df = exc_val_df[(exc_val_df['buying_rate']!='') & (exc_val_df['selling_rate']!='')]
        exc_val_df['currency'] = exc_val_df['currency'].str.strip()
        exc_val_df.loc[exc_val_df['currency']=='USD DENO. $ : 50-100','currency'] = 'USD'
        exc_val_df.loc[exc_val_df['currency']=='JPY : 100','currency'] = 'JPY'
    elif bank_abbv_name == 'KTB TRAVEL':
        options = Options()
        #options.add_argument('ignore-certificate-errors')
        options.headlesss = True
        # options.add_argument("--headless")
        driver = webdriver.Chrome(service=Service('C:\\Users\\thrapoom\\AppData\\Local\\Programs\\Python\\Python311\\Scripts'),options=options)
        driver.get(url)
        exc_val_df = pd.read_html(driver.find_element(By.XPATH,'//table').get_attribute('outerHTML'),encoding = 'utf-8')[0]
        exc_val_df.columns = header
        exc_val_df.loc[exc_val_df["selling_rate"]=='Unq',"selling_rate"] = np.nan
        exc_val_df["selling_rate"] = exc_val_df["selling_rate"].apply(lambda x:"{:.4f}".format(float(x)))
        exc_val_df.loc[exc_val_df["selling_rate"]=='nan',"selling_rate"] = ''
        exc_val_df['bank_abbv_name'] = bank_abbv_name
        exc_val_df = selected_currency(exc_val_df,selected_cur_list)
        ...
        exc_val_df.rename(columns={'CURRENCY':'currency'
                                    ,'BUYING RATE':'buying_rate'
                                    ,'SELLING RATE':'selling_rate'},inplace=True)
        exc_val_df = exc_val_df[['currency','buying_rate','selling_rate']].drop_duplicates('currency',keep='first')
        exc_val_df["selling_rate"] = exc_val_df["selling_rate"].apply(lambda x:"{:.4f}".format(float(x)))
        exc_val_df["buying_rate"] = exc_val_df["buying_rate"].apply(lambda x:"{:.4f}".format(float(x)))
        exc_val_df.loc[:,"currency"] = exc_val_df.loc[:,"currency"].apply(lambda x:x.split('  ')[0])
        exc_val_df["bank_abbv_name"] = bank_abbv_name
        exc_val_df = selected_currency(exc_val_df,selected_cur_list)
    elif bank_abbv_name == 'UOB':
        table = soup.find('table', attrs={'border': '1', 'width': '100%', 'cellspacing': '0', 'cellpadding': '2', 'bordercolor': '#B9DCFF'}).findAll('tr')
        exc_val_list = []
        for row in table:
            try:
                ...
                exc_val_list.append([bank_abbv_name
                            ...
                            # ,"{:.4f}".format(float(row.find_all("td")[2].text))
                            # TT buying
                            ,"{:.4f}".format(float(row.find_all("td")[3].text))
                            # SELL 
                            ,"{:.4f}".format(float(row.find_all("td")[4].text))])
            except: pass
        exc_val_df = pd.DataFrame(exc_val_list,columns = header)
        exc_val_df['currency'] = exc_val_df['currency'].str.replace('JPY (:100)', 'JPY')
    ...
    return exc_val_df
# KBANK , 
kbank_c_df = scrape_website("KBANK JOURNEY","https://www.kasikornbank.com/th/personal/Debit-Card/Pages/exchange-rate.aspx")
bay_c_df = scrape_website("BAY BOARDING","https://www.krungsri.com/th/personal/card/krungsri-boarding-card")
ttb_df = scrape_website("TTB","https://www.ttbbank.com/th/rates/exchange-rates")
ktb_df = scrape_website("KTB TRAVEL","https://exchangerate.krungthai.com/#/travelCardRate")
spr_df = scrape_website("SUPERRICH GREEN","https://www.superrichthailand.com/#!/en/exchange")
uob_df = scrape_website("UOB","https://ereport.uob.co.th/UOBWebFrontService/Exchange/FxRateEnNew.jsp?flags=LastFx")
combined_df = pd.concat([kbank_c_df,bay_c_df,ttb_df,ktb_df,spr_df,uob_df]).reset_index(drop=True)
selected_cur = ktb_df.currency.unique().tolist()
tmp = combined_df.copy()
tmp.rename(columns={'bank_abbv_name':''
                    ,'buying_rate':'Buying Rate'
                    ,'selling_rate':'Selling Rate'
                    ,'currency':'Currency'},inplace=True)
pivoted_df = tmp.pivot(index='Currency', columns='', values=['Buying Rate', 'Selling Rate'])
pivoted_df = pivoted_df[[('Selling Rate','KTB TRAVEL')
                        ,('Selling Rate','TTB')
                        ,('Selling Rate','KBANK JOURNEY')
                        ,('Selling Rate','BAY BOARDING')
                        ,('Selling Rate','SUPERRICH GREEN')
                        ,('Selling Rate','UOB')
                        ,('Buying Rate','KTB TRAVEL')
                        ,('Buying Rate','TTB')
                        #,('Buying Rate','KBANK JOURNEY')
                        ,('Buying Rate','BAY BOARDING')
